...
class Fetch:
    ...
    async def fetch_url(
        ...
        is_page_html = ("<html" in page_raw[:100]
                        or "text/html" in content_type or not content_type)
...
ResumeToolDescription = RichToolDescription(
    ...
    use_when=
    "Puch (or anyone) asks for your resume; this must return raw markdown, \
no extra formatting.",
    side_effects=None,
)
...
async def resume() -> str:
    ...
    try:
        pdf_path = Path("/Users/deshantsingh/Documents/MCP/resume.pdf")
        ...
        markdown = f"# My Resume\n\n{text}"
        logging.info("Resume output:\n%s", markdown)
        return markdown
...
FetchToolDescription = RichToolDescription(
    ...
    use_when=
    "Use this tool when the user provides a URL and asks for its content, or when the user wants to fetch a webpage.",
    side_effects=
    "The user will receive the content of the requested URL in a simplified format, or raw HTML if requested.",
)
...
async def fetch(
    ...
    content, prefix = await Fetch.fetch_url(url_str,
                                            Fetch.USER_AGENT,
                                            force_raw=raw)
    ...
    if start_index >= original_length:
        ...
        if not truncated_content:
            ...
            remaining_content = original_length - (start_index +
...
if __name__ == "__main__":
    ...
    #import sys
    #print("MCP Server started via clauda", file=sys.stderr)
    #asyncio.run(mcp.run())
